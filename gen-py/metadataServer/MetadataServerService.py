...
class Client(Iface):
  ...
  def updateFile(self, f):
    ...
    Parameters:
     - f
    """
    self.send_updateFile(f)
    return self.recv_updateFile()

  def send_updateFile(self, f):
  ...
  def updateDelete(self, f):
    ...
    Parameters:
     - f
    """
    self.send_updateDelete(f)
    return self.recv_updateDelete()

  def send_updateDelete(self, f):
    self._oprot.writeMessageBegin('updateDelete', TMessageType.CALL, self._seqid)
    args = updateDelete_args()
    args.f = f
    args.write(self._oprot)
    self._oprot.writeMessageEnd()
    self._oprot.trans.flush()

  def recv_updateDelete(self):
    (fname, mtype, rseqid) = self._iprot.readMessageBegin()
    if mtype == TMessageType.EXCEPTION:
      x = TApplicationException()
      x.read(self._iprot)
      self._iprot.readMessageEnd()
      raise x
    result = updateDelete_result()
    result.read(self._iprot)
    self._iprot.readMessageEnd()
    if result.success is not None:
      return result.success
    raise TApplicationException(TApplicationException.MISSING_RESULT, "updateDelete failed: unknown result");
...
class Processor(Iface, TProcessor):
  def __init__(self, handler):
    ...
    self._processMap["updateDelete"] = Processor.process_updateDelete
  ...
  def process_updateFile(self, seqid, iprot, oprot):
    ...
    iprot.readMessageEnd()
    result = updateFile_result()
    result.success = self._handler.updateFile(args.f)
    oprot.writeMessageBegin("updateFile", TMessageType.REPLY, seqid)
    result.write(oprot)
    oprot.writeMessageEnd()
    oprot.trans.flush()

  def process_updateDelete(self, seqid, iprot, oprot):
    args = updateDelete_args()
    args.read(iprot)
    iprot.readMessageEnd()
    result = updateDelete_result()
    result.success = self._handler.updateDelete(args.f)
    oprot.writeMessageBegin("updateDelete", TMessageType.REPLY, seqid)
    result.write(oprot)
    oprot.writeMessageEnd()
    oprot.trans.flush()
...
class updateDelete_args:
  ...
  def write(self, oprot):
    if oprot.__class__ == TBinaryProtocol.TBinaryProtocolAccelerated and self.thrift_spec is not None and fastbinary is not None:
      oprot.trans.write(fastbinary.encode_binary(self, (self.__class__, self.thrift_spec)))
      return
    oprot.writeStructBegin('updateDelete_args')
    if self.f is not None:
      oprot.writeFieldBegin('f', TType.STRUCT, 1)
      self.f.write(oprot)
      oprot.writeFieldEnd()
...
class updateDelete_result:
  ...
  def write(self, oprot):
    if oprot.__class__ == TBinaryProtocol.TBinaryProtocolAccelerated and self.thrift_spec is not None and fastbinary is not None:
      oprot.trans.write(fastbinary.encode_binary(self, (self.__class__, self.thrift_spec)))
      return
    oprot.writeStructBegin('updateDelete_result')
    if self.success is not None:
      oprot.writeFieldBegin('success', TType.STRUCT, 0)
      self.success.write(oprot)
      oprot.writeFieldEnd()
