...
class Client(Iface):
  ...
  def hasBlock(self, hash):
    ...
    Parameters:
     - hash
    """
    self.send_hasBlock(hash)
    return self.recv_hasBlock()

  def send_hasBlock(self, hash):
    self._oprot.writeMessageBegin('hasBlock', TMessageType.CALL, self._seqid)
    args = hasBlock_args()
    args.hash = hash
    args.write(self._oprot)
    self._oprot.writeMessageEnd()
  ...
  def recv_hasBlock(self):
    ...
    if mtype == TMessageType.EXCEPTION:
      ...
      self._iprot.readMessageEnd()
      raise x
    result = hasBlock_result()
    result.read(self._iprot)
    self._iprot.readMessageEnd()
    if result.success is not None:
      return result.success
    raise TApplicationException(TApplicationException.MISSING_RESULT, "hasBlock failed: unknown result");
...
class Processor(Iface, TProcessor):
  def __init__(self, handler):
    ...
    self._processMap["hasBlock"] = Processor.process_hasBlock
  ...
  def process_hasBlock(self, seqid, iprot, oprot):
    args = hasBlock_args()
    args.read(iprot)
    iprot.readMessageEnd()
    result = hasBlock_result()
    result.success = self._handler.hasBlock(args.hash)
    oprot.writeMessageBegin("hasBlock", TMessageType.REPLY, seqid)
    result.write(oprot)
    oprot.writeMessageEnd()
    oprot.trans.flush()
...
class hasBlock_args:
  ...
  def write(self, oprot):
    if oprot.__class__ == TBinaryProtocol.TBinaryProtocolAccelerated and self.thrift_spec is not None and fastbinary is not None:
      oprot.trans.write(fastbinary.encode_binary(self, (self.__class__, self.thrift_spec)))
      return
    oprot.writeStructBegin('hasBlock_args')
    if self.hash is not None:
      oprot.writeFieldBegin('hash', TType.STRING, 1)
      oprot.writeString(self.hash)
      oprot.writeFieldEnd()
...
class hasBlock_result:
  ...
  thrift_spec = (
    (0, TType.STRUCT, 'success', (shared.ttypes.hasBlockResponse, shared.ttypes.hasBlockResponse.thrift_spec), None, ), # 0
  )
  ...
  def read(self, iprot):
    ...
    while True:
      ...
      if fid == 0:
        if ftype == TType.STRUCT:
          self.success = shared.ttypes.hasBlockResponse()
          self.success.read(iprot)
        else:
          iprot.skip(ftype)
      else:
  ...
  def write(self, oprot):
    if oprot.__class__ == TBinaryProtocol.TBinaryProtocolAccelerated and self.thrift_spec is not None and fastbinary is not None:
      oprot.trans.write(fastbinary.encode_binary(self, (self.__class__, self.thrift_spec)))
      return
    oprot.writeStructBegin('hasBlock_result')
    if self.success is not None:
      oprot.writeFieldBegin('success', TType.STRUCT, 0)
      self.success.write(oprot)
      oprot.writeFieldEnd()
